...
\fancyfoot[L]{\parbox[t]{0.98\headwidth}{\footnotesize{\sffamily Foreman-Mackey, (2021). \texttt{exoplanet}:
Gradient-based probabilistic inference for exoplanet data \& other
astronomical time
series. \textit{Journal of Open Source Software}, TBD(TBD), TBD. \url{https://doi.org/TBD}}}}
...
\item
  ...
  implementation for \texttt{PyMC3} and \texttt{Theano},
  \texttt{exoplanet-core} includes implementations in \texttt{numpy}
  (Harris et al., 2020) and \texttt{jax} (Bradbury et al., 2018).
\item
...
\item
  ...
  (based on the method used by the \texttt{Stan} project Carpenter et
  al., 2017) that provides better performance on models with correlated
  parameters.
\item
...
(Blunt et al., 2020). These packages all focus on providing a high-level
interface for designing models and then executing a fit.
\texttt{exoplanet}, however, is designed to be lower level and more
conceptually similar to tools like \texttt{batman} (Kreidberg, 2015),
\texttt{PyTransit} (Parviainen, 2015), \texttt{ellc} (Maxted, 2016),
...
formerly known as \texttt{Theano}, Theano Development Team, 2016)
automatic differentiation framework used by \texttt{PyMC3}. This allows
the use of modern inference algorithms such as No U-Turn Sampling
(Hoffman \& Gelman, 2014) or Automatic Differentiation Variational
Inference (Kucukelbir et al., 2017). These algorithms can have some
computational and conceptual advantages over inference methods that do
not use gradients, especially for high-dimensional models. The
...
than finite-difference gradients which can be subject to significant
numerical errors and require 2N computations of a model with N free
parameters.
